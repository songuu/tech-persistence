...
| 类型 | 写入位置 |
|------|---------|
| 解决方案 | `docs/solutions/` + `docs/solutions/index.jsonl` + AGENTS.md / AGENTS.md 投影 |
...
`docs/solutions/*.md` 是唯一详情源；`docs/solutions/index.jsonl` 是唯一摘要索引缓存。AGENTS.md / AGENTS.md 只是 runtime 注入投影，不允许分别手工维护总结。
...
- AGENTS.md / AGENTS.md 的 `### 解决方案索引` managed block 始终保留**最近 5 条**
- always-on 注入恒定，不再线性增长（设计参考 `docs/plans/2026-05-14-claude-md-index-via-prompt-recall.md`）
- 老条目仍可被 **prompt recall hook**（UserPromptSubmit）按用户当轮 prompt 召回（数据源是 `docs/solutions/*.md`，不依赖 AGENTS.md / AGENTS.md 索引）
- 两个 runtime 共享同一套总结内容，只做路径和入口外壳投影

报告中加一行 `Solution index: synced <N> entries → docs/solutions/index.jsonl + AGENTS.md + AGENTS.md`。
