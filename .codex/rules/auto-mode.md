...
| 入口 | 写法 |
|------|------|
| Codex 命令 | `/sprint --auto <参数>` / `/work --auto` / `/review --auto` 等 |
...
- 写入会话外的共享状态：覆盖他人项目、改全局 `~/.codex/settings.json`、改 `~/.codex/config`
- 安全相关：硬编码 secret、关闭安全检查、`--no-verify`、绕过签名
- 风险等级 L4（支付/认证/数据）
- 计划范围与原始请求明显不符（scope creep）
- 测试失败或验证未通过的情况下进入下一阶段
- Goal Loop 迭代级（仅 `--goal --auto`，本协议首个 loop-class 边界）：到 `--max-iter` 硬天花板但目标仍未达成 → 停下问用户，不得自行追加轮次；跨迭代累积 scope creep（单轮看合规、多轮累积偏离原始目标）→ 强制人工
...
`--goal`（目标驱动循环）与 auto-mode 同样正交：`--goal` 决定"是否循环重入直到目标达成"，auto-mode 决定"循环中本应人工的 gate 是否自主跳过"。`--goal` 单独使用不跳任何 gate；只有 `--goal --auto` 才让循环自主推进，且仍受上方 §1 强制人工边界（含 Goal Loop 迭代级边界）约束。
...
| `/sprint --goal` | 循环重入 + Phase gate | `--goal` 单独：gate 全保留（含循环重入需 'go'）；`--goal --auto`：按矩阵自主，但到 max-iter 未达成 / 跨迭代 scope creep 强制问；`--until` 与 max-iter 是确定性终止，优先于 LLM 自评 |
...
不传 `--auto` 时：保持当前的人工 gate 行为，零变化。`--goal` 单独使用同样不触发任何 gate 跳过——目标循环的自主推进必须显式叠加 `--auto`。
