---
description: "多角度代码审查：安全/性能/架构/质量/测试覆盖(风险自适应) + design lens 条件触发 + doc↔code 一致性校验"
---
...
> 以下仅对 Claude Code runtime 生效。Codex CLI 见下方「Multi-runtime fallback」段。
...
3. **强制人工 gate**：即使 `--auto` 模式也必须问用户（与 `~/.claude/rules/auto-mode.md` 强制人工边界一致：destructive / L4 / scope creep / **BLOCKED**）
4. 用户选择继续 / 修复阻塞 / 跳过该视角
...
- destructive / L4 任务相关 P0：即使 `--auto` 仍保留人工 gate（与 `~/.claude/rules/auto-mode.md` 强制人工边界一致）。
...
| runtime | spawn 机制 | 行为 |
|---------|----------|------|
| **Claude Code** | Agent tool + subagent_type + model + prompt | 上述真并行 spawn 协议生效 |
...
### Doc↔Code 一致性 Walkthrough（条件触发）

**目的**：把 [[feedback_drift_fix_requires_full_claim_grep]]「Review 阶段全仓 grep claim」从纪律升为 review 协议步骤，补 `knowledge-drift.js`（ADR-023，pre-commit 只查行号引用**文件存在性**）的**语义盲区**——文档"声称代码做了 X、实际没有"这类 documented-claim-vs-code-reality-drift（本仓库 #1 回归源）。机制形状借自 moai-adk sync-auditor/plan-auditor 的语义 doc↔code↔plan 审计（见 ADR-026 / `docs/solutions/2026-06-15-moai-adk-eval.md` B1），但**降级为 LLM 语义 review lens 而非确定性 enforcement 门**（语义判断不可 hook 确定性化，ADR-017 边界 + measure-before-enforce）。

**触发条件**（任一命中即执行；否则跳过并写一行跳过原因）：

1. diff 含 `*.md` 且含**代码状态断言**：「已修复 / 已实施 / 已下沉 / closed / 由 X 强制 / 自动 Y / enforced by」等声称代码现状的措辞
2. diff **同时改 docs + code**（描述可能与改动不符）
3. plan / sprint / solution 文档 `status` → `completed` / `implemented` / `reviewed`（plan↔实现一致性，对标 plan-auditor）

**审查动作**（主 LLM 语义判断，非确定性门）：对每条代码状态断言，grep / read 被断言的代码，verify claim 与现实是否一致。

**执行规则**（吸收 moai-adk `plan-auditor` / `sync-auditor` 的轻量版 checklist）：

1. **Context isolation**：只信最终 diff / docs / code reality；不把作者解释、对话记忆、"我本来想表达"算作证据。
2. **Evidence required**：`PASS` / `一致` 必须给出 `file:line`、grep/read 锚点或命令输出；无证据不能 PASS。
3. **UNVERIFIED is not PASS**：证据缺失、断言目标不清、grep 范围不足时标 `UNVERIFIED`，按 claim 风险给 P1/P2 action。
4. **Must-pass firewall**：以下任一失败时，walkthrough verdict 直接 `FAIL`，不能被其他通过项抵消：
   - "已实施 / 已下沉 / 已同步 / 已强制 / enforced" 必须有代码或命令证据；
   - `status` 升 `reviewed` / `completed` / `implemented` 必须有验证记录；
   - "没有 X / 不支持 X" 必须展示 grep/read 反证范围。
5. **Findings first, filtering later**：低置信或低严重度可疑 claim 先入表（写 confidence），最终 action 可降级，但不要静默丢弃。

报告在 Findings 前输出：

```markdown
## Doc↔Code 一致性 Walkthrough
| doc claim (file:line) | 断言内容 | code 现实 (grep/read 锚定) | 状态 | confidence | action |
|-----------------------|----------|---------------------------|------|------------|--------|
| docs/plans/X.md:NN | "Y 已下沉 pre-commit" | grep checker = 0 匹配 | FAIL | high | P1: 改措辞或实装 |
```

不一致一律 ≥ P1（虚假 claim 是 doc-drift，#1 回归源）。**双向校验**：既查"doc 高估 code"（声称已做、实际没做），也查"断言 X-缺失 前未 grep"（如写"TP 没有 Y"实则有——本能 [[documented-claim-vs-code-reality-drift]]）。

**Chain-of-Verification（二次自检）**：walkthrough 表完成后，必须再扫一遍 diff/docs 中的 claim words（已 / 无 / 不支持 / 强制 / 自动 / enforced / completed / implemented / reviewed），回答：

- 是否漏查反向 claim（"没有 X"）？
- 是否把文档模式 / 建议误判成已实现机制？
- 是否对所有状态升级行找到验证记录？
- 是否存在 `UNVERIFIED` 被写成 `PASS`？

结果写在 Findings 前：`Second pass: none | added <N> rows`。

**Multi-runtime**：通用（纯 LLM 语义判断，无 spawn 依赖；spawn-capable runtime 可并入 arch reviewer 或单独 spawn，inline-fallback runtime 主 LLM 单 context 自查）。

**measure-not-enforce**：本 walkthrough 是 surface + flag lens，**不下沉 pre-commit**（语义一致性不可确定性判定，ADR-017；其价值经本 lens 在实际 review 中量化后，再议是否把可确定性化的子集下沉，ADR-013 + ADR-022 measure-before-enforce）。
...
6. 用户请求或 sprint 文档含 `1:1` / `pixel` / `像素级` / `fidelity` / `还原不准` / `Figma MCP`
...
**Claude Code spawn 模式**（含 spawn 数 / status 分布 / retry / blocked 字段）：
...
**Runtime gate**：仅 spawn-capable runtime (Claude Code / 等价 Agent tool 可用) 执行；inline-fallback runtime (Codex CLI) 读到此段跳过，使用下方 inline-fallback prompt 模板。
...
审查策略：
1. 优先调用用户级 skill `qa-design-review`（位于 ~/.claude/skills/design-review/SKILL.md；codex 副本中该路径被 regex 替换为 ~/.codex/skills/design-review/SKILL.md，但 codex runtime 不应到达本段——见 runtime gate）
...
A. 设计 token 对齐（与 Figma 1:1 还原 §3.B 同源）
   ...
   - 像素级还原任务缺少 baseline screenshot / actual screenshot / screenshot diff 记录
