---
description: "从当前会话提取技术经验并提交可审计学习候选（/compound 的轻量版）"
---
...
- 自动流程最多执行 `propose`、`evaluate`、`shadow`；不得执行 `approve`、`promote`，不得直接写
  `CLAUDE.md`、rules、instinct、skill、command 或任何 runtime marker。
...
**默认**：所有经验只提出 project-scope candidate；不直接写 `<project>/CLAUDE.md` 或 rules。

**例外**：只有同时满足下列 **全部 5 条** 时，才允许把 candidate 的建议 scope 提升为
personal/global；这仍不授权写入 `~/.claude/CLAUDE.md`：
