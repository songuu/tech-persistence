---
description: "Skill 进化统一入口：diagnose/eval/improve/publish/auto/list"
---
...
diagnose 读 `~/.claude/homunculus/skill-signals/{name}.jsonl`；improve/eval/publish 还必须读取 canonical
...
读取 `~/.claude/homunculus/skill-signals/{name}.jsonl`，分析使用情况。诊断时可半自动从 observations 提取失败 trace（`node scripts/skill-traces.js record ...`，人工确认 gate）供 improve 反思。详细规范见 [skill-diagnose.md](./skill-diagnose.md)（保留 alias）。
