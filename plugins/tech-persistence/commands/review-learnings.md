...
| 召回信号 | `~/.claude/homunculus/telemetry/recall-usage.jsonl` | demand-side 使用率 + 沉睡 domain + MCP 主动检索（measure-only） |
...
读 `~/.claude/homunculus/telemetry/recall-usage.jsonl`（每次会话结束由 Stop hook 追加一条），汇总「注入的知识 domain 实际被碰到了多少」——这是供给侧 `MEMORY.md` 索引覆盖率之外，唯一的需求侧信号。
