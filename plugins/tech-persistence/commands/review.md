...
**审查动作**（主 LLM 语义判断，非确定性门）：对每条代码状态断言，grep / read 被断言的代码，verify claim 与现实是否一致。报告在 Findings 前输出：

```markdown
## Doc↔Code 一致性 Walkthrough
| doc claim (file:line) | 断言内容 | code 现实 (grep/read 锚定) | 一致? | action |
|-----------------------|----------|---------------------------|-------|--------|
| docs/plans/X.md:NN | "Y 已下沉 pre-commit" | grep checker = 0 匹配 | ❌ | P1: 改措辞或实装 |
```
