...
1. 定位项目根，读取 `{{RUNTIME_DIR}}/project-standards.json`；确认 JSON 可解析、schema 受支持，`owner`、`runtime`、
...
5. 核对 `attributes` 指向项目根 `.gitattributes` 的 LF 受管块完整、唯一且位于文件末尾；再核对 `entrypoint`
   指向的 `{{ENTRYPOINT}}` 受管块完整、唯一且引用当前 manifest。`conflicts` 非空时逐项报告，不覆盖冲突文件。
6. 若 sibling runtime 的 `{{SIBLING_RUNTIME_DIR}}/project-standards.json` 存在，使用 `{{SIBLING_ENTRYPOINT}}`
