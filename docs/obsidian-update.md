...
自 2026-06-01 起，**只要 homunculus dir 已是 vault（含 `.obsidian/`），重跑安装即自动刷新** graph.json colorGroups、Dashboard，以及当前 repo `docs/solutions/*.md` 的 vault 投影：
...
刷新是**外科式 + 幂等**：只替换系统管理的 colorGroups（随新增产出类型同步），保留你在图谱界面调的布局偏好（scale/forces 等）；solution 投影只镜像 repo canonical source，删除旧副本；无变化则不写、不产生 `.bak`。下面的手动编辑表仅作为「不重跑安装时」的参考。
