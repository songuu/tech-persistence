...
- `slice-runner.js` 增加 provider 前后 changed-files 快照、文件指纹比较、`ownedFiles` 归属判断，以及 `generated` / `managed` exception 分类。
- `pipeline-providers.js` 在 slice implementation provider 返回后、validation 执行前写入 `changed-files-gate.json`；越界文件会抛错并走 existing implementation failure/block 流程，阻断 `slice-implemented` / `slice-completed`。
- `writeGitDiff()` 现在返回实际 diff 内容，slice 级 `diff.patch` 不再为空。
- slice review prompt 会显式带上 `changed-files-gate.json`，integration/review 链路能按 slice 查看 gate 结果。
- `sliceEvidenceComplete()` 要求 `handoff.json`、`diff.patch`、`changed-files-gate.json`、`validation.json` 同时存在。
- 验证通过：root `agent-orchestrator.js self-test` 覆盖 owned allow、out-of-scope block、generated exception、pre-existing dirty file touch 以及 provider 集成 gate 写入。
...
- `pipeline-state.js` 增加 `transitionRun()` / `transitionSlice()` 作为 run/slice 状态写入口，并在 `state.pipeline.transitionEvents[]` 记录 `from` / `to` / `reason` / `actor` / `source`。
- `pipeline.js` 内部状态推进改为调用 `pipeline-state.js` 的统一 transition helper；dispatch blocked / implementing 两处直接写 `sliceStates[]` 已收口。
- `pipeline-providers.js` 中 global contract、slice planner、slice implementation、slice review、integration review 的状态推进全部改为 transition helper；review approved 路径显式走 `slice-implemented -> slice-reviewed -> slice-completed`。
- gate / validation / handoff parse 在 provider 成功后失败时，会把 provider record 带入 failure artifact，便于 retry 审计。
- self-test 增加 transition event 字段断言，以及 provider 模块禁止直接写 terminal slice/run status 的源码扫描。
- 验证通过：root `agent-orchestrator.js self-test`。
...
- `build-codex-plugin.js` 增加 agent-loop projection 的 provider provenance 保留逻辑，避免把 Claude Code provider 承担的 global contract / integration review 误替换成 Codex。
- `validate-codex-plugin.js` 增加 provider provenance 必备/禁用片段检查，并允许 `skills/agent-loop/SKILL.md` 保留必要的 Claude Code provider 来源说明。
- 验证通过：`node plugins/tech-persistence/scripts/build-codex-plugin.js`、`node scripts/validate-codex-plugin.js`、plugin `agent-orchestrator.js self-test`。
...
- `runtime-paths.js` 增加 `resolvePlanDirectories()` / `resolvePlanPath()` / `resolvePlanWritePath()`；`docs/plans` 是 `sourceOfTruth`，当前 runtime hidden plans 是 `runtimeCache`，另一 runtime hidden plans 是 `legacyFallback`。
- `resolveProjectPlansDir()` 改为返回 `docs/plans`，计划写入默认落 human-readable source of truth。
- `inject-context.js` 的 prototype 状态注入按 source-of-truth → runtime cache → legacy fallback 搜索，并在上下文中标出 `sourceType`。
- `test-inject-context-cost-summary.js` 增加 plan resolver 与 prototype 注入来源测试，`validate-codex-plugin.js` 增加插件投影护栏。
- 验证通过：`node scripts/test-inject-context-cost-summary.js`、`node plugins/tech-persistence/scripts/build-codex-plugin.js`、`node scripts/validate-codex-plugin.js`。
...
- `validate-codex-plugin.js` 现在覆盖 agent-loop auto flag parity、provider provenance、plan source-of-truth helper、`inject-context` projection 和 local require closure。
- 安装验证矩阵发现 `hook-exit-codes.md` 只有生成物、缺 source-of-truth，已补回 `project-level/.claude/rules/hook-exit-codes.md`。
- 验证通过：
  - `node scripts/agent-orchestrator.js self-test`
  - `node scripts/agent-orchestrator.js run --requirement "smoke" --preflight-only`
  - `node scripts/agent-orchestrator.js run --requirement "pipeline smoke" --pipeline --dry-run`
  - `node scripts/test-inject-context-cost-summary.js`
  - `node plugins/tech-persistence/scripts/build-codex-plugin.js`
  - `node scripts/validate-codex-plugin.js`
  - `node scripts/validate-codex-install.js --project`
  - `node scripts/validate-claude-install.js --project`
  - `node plugins/tech-persistence/scripts/agent-orchestrator.js self-test`
  - `git diff --check`（仅 `docs/solutions/index.jsonl` CRLF 提示，无 whitespace error）
