...
| 2026-09-03 | Sprint runtime portability correction | 用户纠正了把 `/sprint` 与 Claude/Codex 固定组合绑定的设计。新增 ADR-041 与运行时决策矩阵：默认由当前可执行宿主闭环，只有 Codex、只有 Claude Code、或两者都没有但存在其他宿主均可继续；非当前 provider 的安装/OAuth 状态不再是 Sprint blocker。`/agent-loop` 明确降为用户显式选择的可选 backend；副作用前允许能力候选 fallback，partial effects 后仍禁止切换 writer。Claude command、Codex-native skill、项目 fallback、plugin 投影与 README 已同步；新增回归 `test-sprint-runtime-portability.js`，focused test、plugin validator、projection boundary、pre-commit 与 diff check 通过。Codex native 全投影测试越过 Sprint <4 KiB 检查后，仍命中既有 compound <4 KiB 基线失败，未归因到本次变更。 |
| 2026-09-03 | Unified Sprint runtime evidence | 新增只读 `/sprint evidence`：以 active/指定 plan 为锚点，分开核证 Harness provider run/Acceptance authority、Transcript 捕获与 ack、当前 Codex JSONL 和 PostgreSQL reader exact readback，统一输出 `harnessUsed/transcriptCaptured/transcriptSynced/sprintTranscriptBound`。无 active Sprint 的宿主记录明确标为 `unbound-*`，不伪装成 Sprint 证据；PG 仅使用 reader 且校验 `transaction_read_only=true`，输出不暴露 secret-bearing URL。source、Claude command、Codex-native reference、plugin utility 与 README 已纳入投影和回归。 |
