...
- [ ] thesis A 被认可有洞察，或被纠偏到更准的方向。
- [ ] 缺陷分层（A1 可测外围 / A2 不可消除内核）清晰，明确哪层可缓解、哪层接受。
- [ ] 若落地，产出的 demand-side 信号**接入一个已有消费点**（否则 dead-on-arrival）。
...
| demand-side usage 信号 | Stop evaluate-session | 启发式匹配 observations vs manifest | `telemetry/memory-recall.jsonl` 扩展字段 | **必须接入消费点**（见下） |

> ⚠️ **集成断裂风险**：若只产 demand-side 信号、无人消费 → dead-on-arrival（[[feedback_enforcement_dead_on_arrival_82pct]]）。**强制收口**：信号必须接入一个已有读取点（候选：`/retrospective` 月度报告，或 SessionStart cost summary 加一行「上次会话 demand-side 使用率」）。无消费点则本 sprint 不做。
...
- [x] **Task 4 [P]**: 测试 `test-recall-usage.js` 16 单测（启发式/边界/脱敏/fail-open，run-tests auto-discover）+ 双 runtime parity（build + validate + 闭包护栏） — 文件: `scripts/test-recall-usage.js` — 风险: L2
...
- `scripts/lib/memory-v5.js`（metric 扩展）
...
| 2026-06-01 | Task 4 | `test-recall-usage.js` 16 单测全过，run-tests auto-discover（20/20）；double-runtime parity build+validate+闭包护栏 ✓ |
...
### 总评
measure-only A1 缓解，零 enforcement，闭环完整。无 P0/P1；2 个 P2 中 1 已补、1 follow-up（归档，与既有 telemetry 同步处理）。18 单测 + 端到端 + 双 runtime parity 全过，pre-commit exit 0。
