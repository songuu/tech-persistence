...
sources:
  ...
  - https://raw.githubusercontent.com/modu-ai/moai-adk/main/.claude/agents/moai/plan-auditor.md
  - https://raw.githubusercontent.com/modu-ai/moai-adk/main/.claude/agents/moai/sync-auditor.md
related:
...
MoAI-ADK = **SPEC-First 重量级生产级 agentic 编码 harness**（Go 单二进制 CLI，38,700+ 行 / 38 包，24-28 agents + 47-52 skills，Plan→Run→Sync→PR 全生命周期 + DB 管理 + design pipeline + GLM tmux 多 LLM）。规模与定位远超 solo 自用、纯 markdown+jsonl 的 TP。
...
> - **completeness critic 挖出 1 真漏 → 后续已实施为 adapt（B1）**：`plan-auditor/sync-auditor` 语义 doc↔code↔plan 审计，**正坐 TP #1 痛点（doc-drift）**。
...
2. **B1 adapt（plan-auditor 语义审计）+ §7.2 发现的 TP 措辞歧义**——eval 反而暴露 TP 自身 doc-drift 防御的语义盲区 + sprint.md:517 一处读起来像虚假 enforcement 的声明。比"借不借机制"更有价值。
...
本 doc 经 13-agent 对抗核验 workflow（refute-by-default + 每条 verdict 强制 grep TP 代码锚定 + completeness critic 查漏）。review pass 完成，status 升 **reviewed**（[[feedback_sibling_eval_completed_status_requires_review_pass]]）。**不升 completed**：R1/R3 仍未做 moai 全仓源码审计；B1 已在 §10 收口，status=reviewed 表示外部源码严格度尚未完全核验。
...
- **R1**：moai 资料来自 README + DeepWiki + GitHub 页面 spot-check，**未做全仓源码审计**。性能 claim（90% rework↓ / 70% bug↓ / 15% 时间↓）是营销数字，未独立核实，本 eval 不依赖它们裁决。
...
- **R3**：Ralph（C5）+ plan-auditor（B1）实现细节未系统读源码。B1 已按**机制形状**落地为 TP 的轻量 `/review` lens；source-read 只影响对 moai 实现严格度的外部评价，不影响 TP 当前落地。
- **R4**：agent / skill 数仍有来源不一致（README intro/title 24 agents + 52 skills，Key Numbers 26 agents + 47 skills，历史 search 28 agents）。不影响裁决，保留 range，不追逐精确营销数字。
...
1. **B1**（plan-auditor 语义审计）——TP doc-drift 防御的真实语义盲区，已按 measure-before-enforce 落为 `/review` lens；
...
- **2026-06-15 source-read follow-up**：用户指定只读 `plan-auditor.md` / `evaluator-active.md`。当前 moai `main` 未找到 `evaluator-active.md`，同目录实际 evaluator 文件为 `sync-auditor.md`；本轮只吸收 4 条 checklist 强化：Context Isolation、UNVERIFIED is not PASS、Must-pass firewall、Chain-of-Verification second pass。拒绝吸收 numeric scoring / evaluator profiles / report-dir / coverage gate 等重型 harness 机制。
...
## 11. 二次检查结论（2026-06-15）

用户请求："`$sprint --auto` 检查本 doc 还有没有可以继续执行和优化的"。

本轮复核结论：**可直接执行的 TP 侧优化已收口；剩余项是外部源码严格度提升，不是当前必须实施项。**

| 项 | 判定 | 依据 | 下一步 |
|----|------|------|--------|
| B1 继续执行 | **已增强完成** | `/review` Doc↔Code 一致性 Walkthrough 已落 user-level / Codex / plugin 副本，ADR-026 已采纳；本轮 source-read 后补强 Context Isolation / UNVERIFIED / must-pass / second-pass checklist | 无需再实现 |
| 文档 stale 修正 | **本轮已做** | §7 仍写 "B1 待最终处置"，§9 仍写 "backlog"，与 §10 已实施冲突 | 已改为 "B1 已收口 / status 仍 reviewed 因 R1/R3" |
| R1/R3 源码细读 | **可选，不阻塞** | 只会提高对 moai 实现质量的信心；TP 已拒绝重实现，只借机制形状 | 仅当要把 status 升 completed 或写更严外部审计时再读 Ralph + plan-auditor 源码 |
| R2 全量 52 skills 枚举 | **不建议** | 目标用户错配 + doc-saturated 模式下 ROI 低；critic 已专审 evaluator 类并挖出唯一真漏 B1 | 不做，除非未来出现 thin-doc product repo 迁移场景 |
| R4 agent/skill 数 | **不追精确值** | GitHub README 自身 intro/title 与 Key Numbers 仍不一致；数字不参与 verdict | 保留 range，避免把营销数字当架构事实 |
| status 升 `completed` | **暂不升** | 外部源码未全审仍是事实；`reviewed` 更诚实 | 维持 `status: reviewed` |
