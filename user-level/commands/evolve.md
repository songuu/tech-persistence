...
- `--auto`：自动审查模式。聚类置信度 ≥ 0.85 且产物为 skill / command（不涉及修改全局 rules、CLAUDE.md、agents）时自动落地；改写 CLAUDE.md / 全局 rules / `~/.claude/agents/` 仍保留人工 gate。详见 `~/.claude/rules/auto-mode.md`。
