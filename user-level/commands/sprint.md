...
- **`--goal --auto`**：循环按 auto-mode 决策矩阵自主推进，但 auto-mode 的**强制人工边界**（destructive / L4 / 安全 / scope creep / 测试失败）以及新增的**迭代级边界**（到 max-iter 仍未达成 → 停下问人；跨迭代累积 scope creep → 强制人工）**无视 --auto**，是真正的断路器。详见 `~/.claude/rules/auto-mode.md`。
