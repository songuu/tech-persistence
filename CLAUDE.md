...
- [2026-05-15] [memory-v5/persona/dual-system-discovery] TDAI sibling-eval §4 借鉴点 1 提前执行——加 `persona.md` 5 字段顶层维度（Role/Preferences/Non-negotiables/Communication style/Known context，body ~875 chars）+ `inject-context.js` section 0c（900 chars 预算，位于 Memory v5 index 之前）+ Codex plugin 副本经 `build-codex-plugin.js` 同步。实施中**发现 TP 同时有两套 memory 系统**：`~/.claude/projects/C--<cwdpath>/memory/`（Claude Code core auto memory，所有现有 `feedback_*`/`user_*` 在此）vs `~/.claude/homunculus/projects/<gitHash>/memory/`（Tech-persistence v5，hook 读 + 双 runtime parity）。第一直觉写到 auto-memory dir → dogfood Persona 段缺失 → 才发现两套并存（[[ADR-011]] multi-runtime parity 历史性违反但此前未发现，所有 `feedback_*`/`user_*` 在 Codex 端不可见）。位置决策：persona.md 只写 v5 dir，避免双重注入且保 Codex 可见；symlink 到 global `~/.claude/persona.md` 未实施（YAGNI）；预算 500→700→900 字符逐步校准（spec 暗示与实际 ~875 char 内容不匹配）。新本能 [[reuse-existing-infra-before-building-new]] 二次应用（复用 `compatReadDirs` + `detectProjectIdentity` 但未及时验证写入目录归属）+ [[ADR-015]] → `docs/solutions/2026-05-15-persona-top-level-dimension.md`
