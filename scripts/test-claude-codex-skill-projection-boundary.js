...
function main() {
  ...
  try {
    ...
    const commandSkillOverlap = claudeSkills.filter((name) => commandNames.includes(name));

    assert.deepStrictEqual(
      claudeSkills,
      [...builder.expectedSkills].sort(),
      'Claude skills must contain only canonical user-level skills'
    );
    assert.deepStrictEqual(
      commandSkillOverlap,
      [],
      'Claude commands must not be duplicated as same-name skills'
    );
