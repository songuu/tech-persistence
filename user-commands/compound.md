...
🧠 本能更新:
   🆕 fix-xxx (debugging, 0.5)
   ⬆️  prefer-yyy (code-style, 0.7 → 0.8)
...
```
/think   → 产出: 范围定义          → 供 /plan 读取
/plan    → 产出: 实现计划          → 供 /work 读取
/work    → 产出: 代码变更          → 供 /review 审查
/review  → 产出: 审查报告 + 修复    → 供 /compound 提取
/compound → 产出: 解决方案+本能+rules → 供下次 /plan 自动读取
                                      ↑ 这就是"复利"
```
