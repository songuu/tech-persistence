...
### ADR-026: moai-adk eval 的 B1（语义 doc↔code↔plan 审计）落地为 `/review` 条件触发 walkthrough（LLM measure lens），不下沉 pre-commit enforcement (2026-06-15)
- **状态**：已采纳（已实施）
- **上下文**：moai-adk sibling-eval（[[2026-06-15-moai-adk-eval]]）11 候选全 reject，但 completeness critic 对抗核验挖出 1 个**未 score 的真漏 B1**：moai 的 `sync-auditor`/`plan-auditor` 做语义 doc↔code↔plan 一致性审计，**正坐 TP #1 痛点 doc-drift**（[[documented-claim-vs-code-reality-drift]]，本仓库 #1 回归源）。验证：TP 的 `knowledge-drift.js`（[[ADR-023]]）pre-commit 只查行号引用**文件存在性**，**无语义层**（测不出"doc 声称 X 已修、code 实际没改"）；`feedback_drift_fix_requires_full_claim_grep` 这条「Review 阶段全仓 grep claim」此前**只是 memory 纪律、未 wire 进 /review 协议**。本次 eval 自身就产出两个 drift 实例（draft C1 "TP 没 EARS" 实则有 EARS-lite；对抗 agent 反向把 sprint.md 项目模式误判为 TP enforcement）——双向 drift 都需机制兜底。用户指令"直接实施 B1"。
- **决策**：B1 落地为 `user-level/commands/review.md` 的 **「Doc↔Code 一致性 Walkthrough」条件触发段**（与既有 Gap Detection Walkthrough 同构），内容：(1) 触发条件 = diff 含 `*.md` 代码状态断言 / 同改 docs+code / plan-solution status→completed-implemented-reviewed；(2) 动作 = 主 LLM 对每条断言 grep/read 被断言代码做语义一致性 verify，输出 walkthrough 表，不一致 ≥P1；(3) **双向校验**（doc 高估 code + 断言缺失前未 grep）；(4) **measure-not-enforce**：是 surface+flag review lens，**明确不下沉 pre-commit**。经 propagate+build 同步 4 副本（Codex parity，[[ADR-024]]）。
- **原因**：(1) 语义 doc↔code 一致性是 **LLM 判断**，无 exit code、hook 看不到——属 [[ADR-017]]「语义信号不可确定性 hook 化」边界，强行做 pre-commit 门会误判（正如本 eval 对抗 agent 把项目模式误当 TP enforcement，确定性 grep 更会 FP）；(2) measure-before-enforce（[[ADR-022]]）：先以 review lens 在真实 review 中量化 doc-drift 命中率，再议是否把可确定性化的子集下沉（[[ADR-013]]），而非未证价值先上 enforcement；(3) 轻量优先（[[ADR-011]] #3）：纯 markdown review 协议加法，零新 infra（拒了 moai 的 Go evaluator agent 实现，只借机制形状）；(4) 把已有纪律 memory（`feedback_drift_fix_requires_full_claim_grep`）升为协议步骤，符合 mechanism-over-discipline 的**测量层**版本（未到 enforcement 层）。
- **备选**：(a) 下沉 pre-commit 做语义 claim 校验——撞 [[ADR-017]] 边界（语义不可确定性判定）+ 未证价值（measure-before-enforce），否决；(b) 扩 `knowledge-drift.js` 加语义比对——同 (a) 不可确定性化，且 pre-commit 是开发期 git hook 不该跑 LLM，否决；(c) 新增第 7 review 视角（spawn 独立 reviewer）——过重，walkthrough（主 LLM 自查步骤）已够轻且与 Gap Detection 同构，否决；(d) 维持 backlog 不实施——用户显式"直接实施"，且坐 #1 痛点价值明确，否决。
- **影响**：(1) `/review`（及 `/sprint` Phase 4 经 /review）今后对触及 docs/plan-status 的变更自动跑 doc↔code 一致性 walkthrough，doc-drift 多一道 review 兜底；(2) 这是 [[ADR-017]] 边界的又一落点——「能确定性 hook 的（文件存在性）下沉 pre-commit（[[ADR-023]]）；语义的（claim 一致性）留 LLM review lens」分层明确；(3) 若该 lens 在实践中证明高频命中且部分可确定性化，再按 [[ADR-013]] 评估下沉子集（measure-before-enforce 的标准路径）；(4) sibling-eval 的 completeness critic 首次产出**已实施**的 adapt（前 5 次 eval 多为 0 借鉴或 deferred），印证对抗核验 + 查漏 critic 的价值（[[feedback_single_pass_eval_lenient_vs_adversarial]]）。
- **来源**：`docs/solutions/2026-06-15-moai-adk-eval.md` B1 + §7，completeness critic 核验。

### ADR-025: adapt#2 `/instinct-mine` 经 T5 spike 自我否决——去重 gate 工作正常但 0/17 net-new 候选通过对抗核验；根因是 doc-saturated repo，冻结设计、触发条件=thin-doc repo 重 spike (2026-06-05)
- **状态**：已采纳（决策内容 = 冻结特性，不并入）
- **上下文**：ECC-eval（[[2026-06-05-ecc-eval]]）仅有的 2 个 adapt 之一 = lean `/instinct-mine`（一次性从 repo 约定挖 instinct 填充沉睡 domain，针对 P0d「instinct domain 多样性低」）。`docs/plans/2026-06-05-ecc-adapts.md` 把它 gated 为 **spike-first**（[[feedback_unproven_protocol_rollback_before_enforcement]]）：施工前先 T5 手跑一次，验证 T7 去重 gate 能挡住已成文约定、且 T8 注入/evolve 护栏不靠误读 `>=0.5` 阈值。本次执行该 spike（4 phase workflow：枚举 mineable 候选 → 去重 gate 干跑 → 对抗核验每个 net-new survivor → 综合裁决；23 agents）。
- **决策**：**冻结 `/instinct-mine`（adapt#2）设计，不推进 T6-T9，不把任何挖出的候选并入 instinct pipeline**。记录 5 点结论：(1) 去重 gate（T7）实测工作正常；(2) 对抗核验后 0/17 net-new 通过 keep；(3) 根因 = 本 repo 文档基近饱和；(4) 任何未来 revival 的 guard 设计 = **both**（`<0.5` confidence block + `source:"repo-curation"` 字段过滤，二者缺一不可）；(5) 显式 deferral——revival 前必须在 thin-doc product repo 重做 spike。
- **原因**：(1) 40 候选 → 去重 tally `{total 39, redundant 22, net_new 17}`，`gate_works=true`——去重 gate 正确剔除了显然成文的一半，**特性不是因为 gate 坏了而失败**；(2) 17 个 net-new 经对抗核验 keep=0：9 个判 metric-gaming（仅为让 demand-side telemetry 的 domain 多样性好看）、6 个 dormant-prone（挖了没人碰）、2 个 marginal；(3) 根因是 TP 本身是「自进化工程系统」这一**文档即交付物**的 meta-tooling repo——约定密集成文于**三层每会话注入的上下文**（全局 `CLAUDE.md`+`rules/`、项目 `CLAUDE.md`+`.claude/rules/` 含 23 条 ADR + ~25 条 gotchas、~30 个 `feedback_*`/`user_*` memory），enforcement / memory-telemetry / agent-loop-skill domain 近饱和；(4) `inject-context.js` 本身不 surface rules/CLAUDE.md（`inject_surfaces_rules=false`），但 host runtime 已每会话注入它们——把约定挖成 instinct 只是经第二通道再注入一份拷贝，**零行为增量**；(5) measure-before-enforce（[[ADR-022]]）：实测此处零验证收益 → 不发任何东西，符合 lightweight-first。
- **备选**：(a) `proceed-to-T6-T9`——需 >0 真 net-new 填沉睡 domain 且有 injection 之外价值，核验找到 0，否决；(b) `proceed-narrowed`（仅挖 survivor 子集 + 严格 source-gate）——需 >0 survivor 有 real value，核验 keep=0，否决；(c) 直接弃用不记录——违反 anti-rediscovery（未来 cycle 会重提同特性、重跑同一昂贵 spike），故必须 ADR 记录，正是 [[documented-claim-vs-code-reality-drift]] 关切的反面；(d) 丢弃 ECC 的 Python daemon + Haiku + 1826 行 instinct-cli + auto-promote——在 eval 阶段已因违反 lightweight-first + parity 否决，本 ADR 不重议。
- **影响**：(1) **sample bias 警告**——`0-survivor` 是 repo-specific 结论（doc-saturated meta-tooling repo），**不是「机制无价值」的普适裁决**；`/instinct-mine` 在文档稀薄的普通 product repo 上可能有真价值，revival 前必须先在那类 repo 重 spike；(2) 任何 revival 的 guard 必须是 both——`<0.5` confidence block 是必要但**不充分**（`sub_0_5_blocks_injection=true` 但 `source_gate_needed=true`：0.50 边界 + `/evolve` 可能升权），必须叠加 source 字段过滤直到 session-confirmed；(3) spike 暴露一个**真实但 mine 没命中的 doc gap**——`api-conventions.md` / `performance.md` / `testing-patterns.md` 是 header-only 空模板，gap 真实但 `/instinct-mine` 没挖出可填的，故该 gap 另行处理（不靠 mine）；(4) 今后任何「挖掘/聚合类知识通道」提案，先问「本 repo 的目标知识是否已在 session-injected 层成文」——若是则 dedup 后多半零 yield，spike-first 是正确 gate。
- **来源**：`docs/plans/2026-06-05-ecc-adapts.md`（adapt#2 T5 spike 结果），`docs/solutions/2026-06-05-ecc-eval.md`（上游评估）。
...
- **决策**：(1) block 档收窄到**「带行号(`:NN`) + 源码前缀(scripts/docs/plugins/user-level) + 文件不存在」**——作者标行号 + 源码目录 = 明确指向 repo 内具体代码位置（强 drift 信号），dogfood 探针 v2 证明此集现有 **0 FP**，故**无需 grandfather**（区别于 [[ADR-012]] plan lint 的 filename-date grandfather）；(2) 行号**值绝不校验**（必随编辑漂移，校验=持续 FP）——只校验文件存在；(3) 裸文件名带行号 → glob basename（≥1 匹配=ok，0=warn **不阻塞**，不要求唯一：`pipeline.js` 有 plugin 副本属正常）；(4) `...` 简写 / `*` / 运行时前缀(.claude//.codex/) / **无行号路径引用** → skip（确定性不可判，宁可 FN 不可 FP）；(5) inline-code 符号存在性校验 **deferred**（grep FP 高，匹配注释/通用词）；(6) 纯函数 lib（`knowledge-drift.js`）+ checker IO 分离（`git ls-files` 建 known 索引传入），**不进 plugin**（pre-commit 是开发期 git hook，不在 `copyUtilityScripts`，无 [[ADR-020]] parity 负担）。
- **原因**：(1) enforcement 的 FP 必须靠**真实语料 dogfood** 压制而非智能猜测（[[ADR-013]] §B：naive 方案 FP 率 95%，收窄后 0%——第三次成功应用该协议）；(2) 文档天然合法引用未来/假设/运行时文件，"路径存在性"**不是确定性可判信号**——只有"带行号 + 源码前缀"才是作者对 repo 现状的确定性断言；(3) **FN-over-FP 原则**（[[ADR-013]] §B）：漏检（裸名多副本、无行号引用）比误拒合法 commit 安全，因误拒催生 `--no-verify` 习惯导致 enforcement 死亡（[[feedback_enforcement_dead_on_arrival_82pct]]）；(4) lib/checker IO 分离便于纯函数单测 + 固化 dogfood gate 防回归。
- **备选**：(a) 全路径存在性校验——dogfood 40 FP，否决；(b) 校验行号值——漂移持续 FP，否决；(c) 符号 grep 校验——FP 高，deferred；(d) filename-date grandfather——rules 无 date 前缀无法 grandfather，且收窄后 0 FP 本就无需，否决；(e) 进 plugin parity——pre-commit 非 runtime，无需。
- **影响**：(1) pre-commit-check 新增第 7 个 checker（继 [[ADR-013]] 的 checkPropagateSync 等 6 个），引入 pre-commit **首个非阻塞 warn 档**（block 计入 exit 1、warn 总打印但 exit 0）；(2) 今后给 knowledge 层加 enforcement 先问"这个信号确定性可判吗"——路径存在性只在"带行号 + 源码前缀"子集可判，这是 [[ADR-016]]/[[ADR-017]]/[[ADR-021]]「确定性边界」系列的知识层落点；(3) dogfood gate 固化于 `test-knowledge-drift.js`（断言现有 block=0 防误拒回归）；(4) 缺陷 E（roadmap P1）done，B/C/D 仍 planned。
- **来源**：`docs/plans/2026-06-01-knowledge-drift-checker.md`，[[2026-06-01-secondary-defects-roadmap]]。

### ADR-022: agentic coding 趋势下最大缺陷是"纪律+记忆寄生于模型自觉"；分 A1（启发式 demand-side telemetry 可测）/ A2（语义遵守 hook 不可测，接受）两层，measure-before-enforce (2026-06-01)
- **状态**：已采纳
- **上下文**：`docs/plans/2026-06-01-architecture-defect-analysis.md` 分析"当前架构针对 vibe/agentic coding 最大缺陷"。结论缺陷 A：think→plan→work→review→compound 全是 model-driven markdown 协议（[[ADR-021]] 已证 `/sprint` 无宿主进程），知识层产出（instinct/rules/memory）靠 SessionStart 被动注入 12KB（`inject-context.js:28` `CONTEXT_BUDGET_CHARS`），"沉淀→被读→被遵守→改变行为"每环无进程级强制。实测铁证：现有 recall telemetry 的 `hit_rate = indexed_entries / total_entries`（`test-memory-recall-telemetry.js:77`）是**供给侧覆盖率**（塞了多少进索引），完全无需求侧信号（注入的知识有没有被用）。agentic 三趋势（更长自主循环 / 更频繁 /compact / 更多并行 subagent）精确侵蚀这条链路。次级缺陷 B（被动注入 vs 主动检索）/ C（同源验证盲区）/ D（80/20 规划遇执行趋近免费）/ E（知识抗腐化弱）见 plan。
- **决策**：(1) 缺陷 A 分两层处置——**A1 可测外围**（供给有统计、需求无统计）用启发式 demand-side telemetry 缓解：SessionStart 写 `injected manifest`（注入的 instinct domain 集合）→ Stop 读 manifest + 本会话 observations 经**宽松** `inferSessionDomains` 推断触及 domain → 交集算 `usage_rate` + `dormant_domains`（注入了但没碰到 = 退化核心信号）；**A2 不可消除内核**（模型是否"语义遵守"注入纪律）是语义判断、无 exit code、hook 看不到（[[ADR-017]] 边界）→ 显式接受为 model-driven + 4 不可妥协（[[ADR-011]]）固有代价，写入 ADR 停止假装能修。(2) **纯 measure，零 enforcement**（守 [[ADR-013]]/[[ADR-021]]/[[feedback_unproven_protocol_rollback_before_enforcement]]：先量化价值再决定下沉）。(3) 信号**强制接入消费点**防 dead-on-arrival（[[feedback_enforcement_dead_on_arrival_82pct]]）：cost summary 高频（SessionStart 附 prior-session 行）+ `/review-learnings --recall` 低频全量。(4) demand-side 与供给侧 `memory-recall.jsonl` **分离**（独立 `telemetry/recall-usage.jsonl`，Stop 写，避免污染供给侧聚合）。
- **原因**：(1) 语义遵守确定性化是 [[ADR-016]]/[[ADR-017]] 边界的延伸——能 hook 自动派生的是"工具调用 + 启发式 domain"，"模型有没有真按本能行动"必须语义判断、强行 hook 化产生噪声；(2) measure-before-enforce 是 [[ADR-013]] mechanism-over-discipline 的**前提**（无数据的 enforcement 是盲下沉）；(3) `inferSessionDomains` 宽松（宁可多算碰到、少误报沉睡）保证 dormant 假阳性方向安全；(4) 复用现有 telemetry 写入 + 脱敏（`redaction.js` `stripPrivateTags`）设施（[[reuse-existing-infra-before-building-new]]）。
- **备选**：(a) 给 sprint 加进程级强制让模型必遵守——无宿主进程做不到（[[ADR-021]] 已定论）且撞轻量；(b) 精确匹配每条 instinct 是否被语义遵守——[[ADR-017]] 边界不可达；(c) 直接基于 dormant 自动裁剪本能——未证价值就上 enforcement，撞 [[feedback_enforcement_dead_on_arrival_82pct]]，列为 deferred（证价值后再做）；(d) demand-side 混入 `memory-recall.jsonl`——污染供给侧聚合，否决。
- **影响**：(1) demand-side telemetry 是 A1 缓解，A2 是已接受的架构张力——今后讨论"知识有没有被用"先看 `recall-usage.jsonl`，讨论"模型有没有遵守"承认不可确定性测；(2) 基于 dormant 信号的裁剪 enforcement 是 **deferred**（证价值后下沉，守 measure-before-enforce）；(3) `recall-usage.js` 是 hook lib（被 inject/evaluate require → 进 `hooks/lib` + plugin 副本，经 `copyHookLibs` glob 自动复制，[[ADR-020]] 机制，validate 闭包护栏已覆盖）；(4) **实测 finding**：本仓库 15 个注入 instinct 去重后 domain 仅 `workflow`/`code-style`（`detectPatterns` 主产 workflow/debugging domain）——instinct domain 多样性低限制 demand-side 区分度，是 A1 真实上限，但仍优于零信号。
- **来源**：`docs/plans/2026-06-01-architecture-defect-analysis.md`。

### ADR-021: model-driven markdown 协议的"确定性强制"上限由"是否存在宿主进程/可挂事件"决定；`/sprint --goal` 自主循环用 pure-prompt MVP，max-iter 硬天花板下沉为确定性 helper 条件推迟 (2026-05-29)
- **状态**：已采纳
- **上下文**：`docs/plans/2026-05-29-sprint-goal-mode.md` 给 `/sprint` 加 `--goal` 自主目标驱动循环（设目标 → think→plan→work→review→compound 重入直到达成或终止），用户 4 维控制（终止条件/自动化/范围约束/运行时），且 `确定性优先` 是不可妥协原则。Plan 阶段对抗设计 panel（pure-prompt vs deterministic-helper vs hybrid）暴露 load-bearing 事实：**`/sprint` 没有宿主进程**——phase 重入是「模型读 markdown 自选继续」，对比 `pipeline.js:472` 真实 Node `for` 循环（`maxIterations=32` 由进程强制计数）。无 hookable per-iteration 事件。推论：一个「模型必须自愿调用」的 max-iter CLI 计数器**并不比 prose 更被强制**，只是把信任从「数对」挪到「记得调用」（adversary 自标 helper 有 HIGH 级 voluntary-call gap）。同时 `确定性优先` 与 [[feedback_unproven_protocol_rollback_before_enforcement]]（2026-05-22 预热协议 rollback 活案例）在此真冲突——后者要求未证协议先证价值再下沉 enforcement。
- **决策**：(1) `--goal` 用 **pure-prompt 协议 MVP** 实现（零代码，与 `--auto`/`--caveman`「协议 + auto-mode.md 引用、无 backing code」同构）；确定性靠"frontmatter 持久 `goal_iteration` + 每轮强制重读磁盘（不信 /compact 后对话记忆）+ 强制打印 check 行（跳过在 transcript 可见）+ 低默认 max-iter=3 + auto-mode 迭代级强制 gate"多层，**明文声明这是协议层可达上限、非进程级硬计数**；(2) 终止优先级确定性压倒语义：`--until` exit0 / `iteration>=max-iter` 硬停，LLM 达成自评仅 advisory（可提前停、不可越天花板）；(3) `--goal` 单用**不开自主**，自主须显式 `--goal --auto`（守 [[feedback_no_auto_default]]），helper 即便将来落地也 inert until --auto；(4) max-iter 下沉为 `scripts/sprint-goal.js` 确定性 helper（lib+CLI，蓝图冻结于 plan 附录 A）**条件推迟**——仅当 `--goal --auto` 被证明高频且有价值再做；(5) `--runtime both` 跨运行时委托 agent-loop，本版本仅文档化（agent-loop 无 goal-budget 可承接，需另设计 seam）。
- **原因**：(1) 确定性的可达性受架构决定——有宿主进程/hookable 事件才能真强制（[[ADR-016]] skill-publish guard 守的是产生 git commit 的动作，有确定性触发点；本特性的循环边界是模型自选重入，无此触发点），对无触发点的环节堆 CLI 是"guarding the unguardable"；(2) `--goal` 是未证特性，helper 的边际确定性真实但小（voluntary-call gap），代价是 [[ADR-020]] 全链 + 3 档负样本 + parity 表面——[[feedback_unproven_protocol_rollback_before_enforcement]] + [[ADR-013]] 谨慎要求先证价值；(3) pure-prompt 干净可撤（git revert 几个 md），与既有 flag 机制同构，认知成本低。
- **备选**：(a) hybrid（仅 --auto 路径加最小 helper）——adversary 推荐，但 voluntary-call gap 使其也非真强制，且给未证特性引入 ADR-020 全链，否决（条件推迟保留为 follow-up）；(b) full helper 永远走 CLI——adversary 标默认人工 gate 路径 over-engineering（人工已是天花板），否决；(c) 纯 LLM 终止判定无 max-iter 兜底——失控风险，违反确定性优先，否决。
- **影响**：(1) 今后给 **model-driven 子系统**（markdown 命令/skill 协议）设计"确定性 enforcement"前，先问「有没有宿主进程 / hookable 事件可挂」——无则确定性上限是"协议 + 持久化 + 可见打印"，别堆模型必须自愿调用的 CLI 假装强制；这是 [[ADR-016]]/[[ADR-017]]「语义 vs 确定性边界」的**第三轴**（前两轴：能否 hook 自动派生、是否语义判断；本轴：是否有强制触发点）；(2) `--goal` 的 helper 落地是"证明价值后下沉 mechanism"的 backlog（plan frontmatter `deferred`，deadline 2026-08-29），附录 A 蓝图已冻结避免重新勘察；(3) 未来 sprint 与 `--goal` 循环交互时，max-iter 是协议天花板不是进程保证，审查需确认 frontmatter 重读 + 打印 check 行未被省略；(4) `--runtime both` 是 deferred，agent-loop 未改。
- **来源**：`docs/plans/2026-05-29-sprint-goal-mode.md`（Plan 阶段对抗设计 panel + 选型决策），`docs/solutions/2026-05-29-sprint-goal-mode.md`。
